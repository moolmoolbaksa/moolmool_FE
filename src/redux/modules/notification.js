...
export const getNotiApi = createAsyncThunk(
    ...
    async () => {
        try {
            const response = await axios.get(`https://langho968.shop/api/notifications`,{
                headers: {
...
export const getBarterNotiApi = createAsyncThunk(
    ...
    async ({notificationId, changeId}) => {
        try {
            const response = await axios.get(`https://langho968.shop/api/notification/${notificationId}/decision?baterId=${changeId}`,{
                headers: {                                 
                    Authorization: localStorage.getItem('token'),
                }
            });
...
export const getScoreNotiApi = createAsyncThunk(
    ...
    async ({notificationId, chageId}) => {
        try {
            const response = await axios.get(`https://langho968.shop/api/notification/${notificationId}/score?barterId=${chageId}`,{
                headers: {
                    Authorization: localStorage.getItem('token'),
                }
            });
