    ...
    async (itemId, thunkAPI) => {
        try {
            ...
            if(thunkAPI.getState().user.is_login){
                response = await axios.get(`http://13.125.220.67:8080/api/items/${itemId}`,{
                    headers: {
...
export const deleteProductApi = createAsyncThunk(
    ...
    async (itemId) => {
        try {
            await axios.delete(`http://13.125.220.67:8080/api/items/${itemId}`,{
                headers: {
                    Authorization: localStorage.getItem('token'),
                }
            });
...
export const setTradeApi = createAsyncThunk(
    ...
    async (_,thunkAPI) => {
        ...
        try {
            await axios.post(`http://13.125.220.67:8080/api/trade`,{userId, itemId, myItemIds},{
                headers: {
                    Authorization: localStorage.getItem('token'),
                }
            });
