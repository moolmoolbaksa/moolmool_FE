...
export const setProductScrabApi = createAsyncThunk(
    ...
    async (itemId) => {
        try {
            await axios.post(`${process.env.REACT_APP_SERVER_URL}/api/${itemId}/scrabs`,{},{
                headers: {
                    Authorization: localStorage.getItem('token'),
                }
            });
...
export const getTradeProductApi = createAsyncThunk(
    ...
    async ({itemId, userId}) => {
        try {
            const response = await axios.get(`${process.env.REACT_APP_SERVER_URL}/api/trade?itemId=${itemId}&userId=${userId}`,{
                headers: {
                    Authorization: localStorage.getItem('token'),
                } 
            });
...
export const setTradeApi = createAsyncThunk(
    ...
    async (_,thunkAPI) => {
        ...
        try {
            await axios.post(`${process.env.REACT_APP_SERVER_URL}/api/trade`,{userId, itemId, myItemIds},{
                headers: {
                    Authorization: localStorage.getItem('token'),
                }
            });
