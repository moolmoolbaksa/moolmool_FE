...
export const getBarterNotiApi = createAsyncThunk(
    ...
    async ({notificationId, changeId}) => {
        try {
            const response = await axios.get(`${process.env.REACT_APP_SERVER_URL}/api/notification/${notificationId}/decision?baterId=${changeId}`,{
                headers: {                                 
                    Authorization: localStorage.getItem('token'),
                }
            });
...
export const getScoreNotiApi = createAsyncThunk(
    ...
    async ({notificationId, chageId}) => {
        try {
            const response = await axios.get(`${process.env.REACT_APP_SERVER_URL}/api/notification/${notificationId}/score?barterId=${chageId}`,{
                headers: {
                    Authorization: localStorage.getItem('token'),
                }
            });
