...
export const getNotiApi = createAsyncThunk(
    ...
    async () => {
        try {
            const response = await axios.get(`http://13.125.220.67:8080/api/notifications`,{
                headers: {
...
export const getBarterNotiApi = createAsyncThunk(
    ...
    async ({notificationId, changeId}) => {
        try {
            const response = await axios.get(`http://13.125.220.67:8080/api/notification/${notificationId}/decision?baterId=${changeId}`,{
                headers: {                                 
                    Authorization: localStorage.getItem('token'),
                }
            });
...
export const getScoreNotiApi = createAsyncThunk(
    ...
    async ({notificationId, chageId}) => {
        try {
            const response = await axios.get(`http://13.125.220.67:8080/api/notification/${notificationId}/score?barterId=${chageId}`,{
                headers: {
                    Authorization: localStorage.getItem('token'),
                }
            });
